...
public class Shooter extends NRSubsystem {
    ...
    private Shooter() {
        if (EnabledSubsystems.SHOOTER_ENABLED) {
            ...
            // shooterTalon1.configPeakCurrentDuration(CONTINUOUS_CURRENT_LIMIT_SHOOTER,
            // DEFAULT_TIMEOUT);
            // shooterTalon2.enableCurrentLimit(true);
            // shooterTalon2.configPeakCurrentLimit(PEAK_CURRENT_SHOOTER, DEFAULT_TIMEOUT);
            // shooterTalon2.configPeakCurrentDuration(CONTINUOUS_CURRENT_LIMIT_SHOOTER,
            // DEFAULT_TIMEOUT);
    ...
    public void smartDashboardInfo() {
        if (shooterTalon1 != null && shooterTalon2 != null) {
            if (EnabledSubsystems.SHOOTER_SMARTDASHBOARD_BASIC_ENABLED) {
                ...
                // getSpeedShooter2().get(Angle.Unit.DEGREE, Time.Unit.SECOND));
            ...
            if(goalSpeed.get(Angle.Unit.ROTATION, Time.Unit.MINUTE) > MAX_SPEED.get(Angle.Unit.ROTATION, Time.Unit.MINUTE))
                goalSpeed = MAX_SPEED;
        }
    }
    ...
    public AngularSpeed getSpeedShooter1() {
        if (shooterTalon1 != null) {
            return new AngularSpeed(shooterTalon1.getSelectedSensorVelocity(), Angle.Unit.SHOOTER_ENCODER_TICK,
    ...
    public AngularSpeed getSpeedShooter2() {
        if (shooterTalon2 != null) {
            return new AngularSpeed(shooterTalon2.getSelectedSensorVelocity(), Angle.Unit.SHOOTER_ENCODER_TICK,
                    Time.Unit.HUNDRED_MILLISECOND);
        }
        return AngularSpeed.ZERO;
    }
    ...
    public void setMotorSpeed(AngularSpeed speed) {
        if (shooterTalon1 != null) {
            if(speed.get(Angle.Unit.ROTATION, Time.Unit.MINUTE) > MAX_SPEED.get(Angle.Unit.ROTATION, Time.Unit.MINUTE))
                speed = MAX_SPEED;
            // SmartDashboard.putNumber("Commanding Shooter Speed", 1706);
            speedSetPointShooter = speed;
            shooterTalon1.selectProfileSlot(VEL_SLOT, PID_TYPE);
            shooterTalon1.set(ControlMode.Velocity,
                    speed.get(Angle.Unit.SHOOTER_ENCODER_TICK, Time.Unit.HUNDRED_MILLISECOND));
            shooterTalon2.selectProfileSlot(VEL_SLOT, PID_TYPE);
            shooterTalon2.set(ControlMode.Velocity,
                    speed.get(Angle.Unit.SHOOTER_ENCODER_TICK, Time.Unit.HUNDRED_MILLISECOND));
        }
    }
    ...
    {
        if(getSpeedShooter1().get(Angle.Unit.ROTATION, Time.Unit.MINUTE) > MAX_SPEED.get(Angle.Unit.ROTATION, Time.Unit.MINUTE))
